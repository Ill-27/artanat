...
module.exports = (req, res) => {
    try {
        ...
        // Отправляем JavaScript с эффектом звездного поля и водяными знаками
        res.send(`
// ================= ЗАЩИТА ================= //
...
watermarkDiv.id = 'arterii-watermark';
...
watermarkDiv.appendChild(watermarkRenderer.domElement);
...
class SeamlessWatermark {
    ...
    setupProtection() {
        setInterval(() => {
            if (!document.getElementById('arterii-watermark').contains(watermarkRenderer.domElement)) {
                document.getElementById('arterii-watermark').appendChild(watermarkRenderer.domElement);
            }
        }, 1000);
